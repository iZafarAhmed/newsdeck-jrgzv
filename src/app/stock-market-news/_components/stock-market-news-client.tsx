...
interface StockMarketNewsClientProps {
  sources: StockMarketNewsSource[];
}

type SortableColumn = keyof StockMarketNewsSource;
...
export function StockMarketNewsClient({ sources }: StockMarketNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<StockMarketNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
        bValue = b[sortColumn as keyof Omit<StockMarketNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Stock Market News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top stock market news sources from around the world.
        </p>
      </div>
