...
import type { NewsSite as UKSportsNewsSource } from "@/data/all-news-sources";
import { Button } from "@/components/ui/button";

interface UKSportsNewsClientProps {
  sources: UKSportsNewsSource[];
}

type SortableColumn = keyof UKSportsNewsSource;
...
export function UKSportsNewsClient({ sources }: UKSportsNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<UKSportsNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
        bValue = b[sortColumn as keyof Omit<UKSportsNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">UK Sports News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top UK sports news sources.
        </p>
      </div>
