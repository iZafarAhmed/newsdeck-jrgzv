...
import type { NewsSite as WyomingNewsSource } from "@/data/all-news-sources";
import { Button } from "@/components/ui/button";

interface WyomingNewsClientProps {
  sources: WyomingNewsSource[];
}

type SortableColumn = keyof WyomingNewsSource;
...
export function WyomingNewsClient({ sources }: WyomingNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<WyomingNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
        bValue = b[sortColumn as keyof Omit<WyomingNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Wyoming News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top news sources from Wyoming.
        </p>
      </div>
