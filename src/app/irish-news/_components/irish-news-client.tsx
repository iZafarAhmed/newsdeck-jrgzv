...
import type { NewsSite as IrishNewsSource } from "@/data/all-news-sources";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";

interface IrishNewsClientProps {
  sources: IrishNewsSource[];
}

type SortableColumn = keyof IrishNewsSource;
...
const getTotalFollowers = (source: IrishNewsSource) => {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<IrishNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
        bValue = b[sortColumn as keyof Omit<IrishNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
      }
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        ...
        <p className="text-lg text-muted-foreground mt-2">
          Explore top news sources from Ireland.
        </p>
      </div>
