...
import type { AustralianSportsNewsSource } from "@/data/australian-sports-news-sources";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";

interface AustralianSportsNewsClientProps {
  sources: AustralianSportsNewsSource[];
}

type SortableColumn = keyof AustralianSportsNewsSource;
...
const getTotalFollowers = (source: AustralianSportsNewsSource) => {
    return parseFollowers(source.facebookFollowers) + parseFollowers(source.xFollowers) + parseFollowers(source.instagramFollowers);
}

export function AustralianSportsNewsClient({ sources }: AustralianSportsNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<AustralianSportsNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
        bValue = b[sortColumn as keyof Omit<AustralianSportsNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Australian Sports News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top sports news sources from Australia.
        </p>
      </div>
