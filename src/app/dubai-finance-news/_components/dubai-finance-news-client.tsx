...
import type { NewsSite as DubaiFinanceNewsSource } from "@/data/all-news-sources";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";

interface DubaiFinanceNewsClientProps {
  sources: DubaiFinanceNewsSource[];
}

type SortableColumn = keyof DubaiFinanceNewsSource;
...
const getTotalFollowers = (source: DubaiFinanceNewsSource) => {
    return parseFollowers(source.facebookFollowers) + parseFollowers(source.xFollowers) + parseFollowers(source.instagramFollowers);
}

export function DubaiFinanceNewsClient({ sources }: DubaiFinanceNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<DubaiFinanceNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
        bValue = b[sortColumn as keyof Omit<DubaiFinanceNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Dubai Finance News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top finance news sources from Dubai.
        </p>
      </div>
            ...
            {filteredAndSortedSources.map((source, index) => (
              <TableRow key={source.name} className="hover:bg-muted/50">
                ...
                <TableCell className="text-muted-foreground">
                  {source.facebookUrl && source.facebookFollowers !== 'N/A' && source.facebookUrl !== '#' ? (
                ...
                <TableCell className="text-muted-foreground">
                  {source.xUrl && source.xFollowers !== 'N/A' && source.xUrl !== '#' ? (
                ...
                <TableCell className="text-muted-foreground">
                  {source.instagramUrl && source.instagramFollowers !== 'N/A' && source.instagramUrl !== '#' ? (
