...
import type { EuropeanNewsSource } from "@/data/european-news-sources";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";

interface ScottishNewsClientProps {
  sources: EuropeanNewsSource[];
}

type SortableColumn = keyof EuropeanNewsSource;
...
export function ScottishNewsClient({ sources }: ScottishNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    if (sortColumn === 'facebookFollowers' || sortColumn === 'xFollowers' || sortColumn === 'instagramFollowers') {
        aValue = parseFollowers(a[sortColumn]);
        bValue = parseFollowers(b[sortColumn]);
    } else {
        aValue = a[sortColumn as keyof Omit<EuropeanNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl'>] ?? '';
        bValue = b[sortColumn as keyof Omit<EuropeanNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl'>] ?? '';
    }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Scottish News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          A collection of popular news sources from Scotland.
        </p>
      </div>
