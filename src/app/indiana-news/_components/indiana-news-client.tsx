...
import type { NewsSite as IndianaNewsSource } from "@/data/all-news-sources";
import { Button } from "@/components/ui/button";

interface IndianaNewsClientProps {
  sources: IndianaNewsSource[];
}

type SortableColumn = keyof IndianaNewsSource;
...
export function IndianaNewsClient({ sources }: IndianaNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<IndianaNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
        bValue = b[sortColumn as keyof Omit<IndianaNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Indiana News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top news sources from Indiana.
        </p>
      </div>
