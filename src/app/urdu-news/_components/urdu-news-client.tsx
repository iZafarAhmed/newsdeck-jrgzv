...
import type { NewsSite as UrduNewsSource } from "@/data/all-news-sources";
import { Button } from "@/components/ui/button";

interface UrduNewsClientProps {
  sources: UrduNewsSource[];
}

type SortableColumn = keyof UrduNewsSource;
...
export function UrduNewsClient({ sources }: UrduNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<UrduNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
        bValue = b[sortColumn as keyof Omit<UrduNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Urdu News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top Urdu news sources.
        </p>
      </div>
