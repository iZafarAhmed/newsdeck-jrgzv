...
import type { NewsSite as WashingtonNewsSource } from "@/data/all-news-sources";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";

interface WashingtonNewsClientProps {
  sources: WashingtonNewsSource[];
}

type SortableColumn = keyof WashingtonNewsSource;
...
const getTotalFollowers = (source: WashingtonNewsSource) => {
    return parseFollowers(source.facebookFollowers) + parseFollowers(source.xFollowers) + parseFollowers(source.instagramFollowers);
}

export function WashingtonNewsClient({ sources }: WashingtonNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<WashingtonNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
        bValue = b[sortColumn as keyof Omit<WashingtonNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Washington News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top news sources from Washington.
        </p>
      </div>
