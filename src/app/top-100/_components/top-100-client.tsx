...
import { ExternalLink, Search, ArrowUpDown, Facebook, Instagram, Youtube } from "lucide-react";
import type { Top100NewsSource } from "@/data/top-100-news";
import { Button } from "@/components/ui/button";

interface Top100ClientProps {
  sources: Top100NewsSource[];
}

type SortableColumn = keyof Top100NewsSource | 'facebookFollowers' | 'xFollowers' | 'instagramFollowers';
...
export function Top100Client({ sources }: Top100ClientProps) {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = sources.filter(
        (source) =>
          ...
          (source.format && source.format.toLowerCase().includes(lowercasedSearchTerm))
      );
    }
    ...
    if (sortColumn) {
      const sorted = [...filtered].sort((a, b) => {
        ...
        if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
          ...
          aValue = a[sortColumn as keyof Omit<Top100NewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
          bValue = b[sortColumn as keyof Omit<Top100NewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
        }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">USA News</h1>
      ...
      <div className="max-w-xl mx-auto mb-8">
        <div className="relative">
          ...
          <Input
            type="text"
            placeholder="Search by name, description, or format..."
          ...
          <TableHeader className="bg-muted/50">
            <TableRow>
              ...
              <TableHead>
               ...
               <TableHead>
                <Button variant="ghost" onClick={() => handleSort('format')} className="px-0 hover:bg-transparent">
                  <span className="text-base font-semibold">Format</span>
                  {renderSortArrow('format')}
                </Button>
              </TableHead>
              <TableHead>
              ...
              <TableHead className="text-center text-base font-semibold">YouTube</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {filteredAndSortedSources.map((source, index) => (
              <TableRow key={source.name} className="hover:bg-muted/50">
                ...
                <TableCell className="text-muted-foreground">{source.format || 'N/A'}</TableCell>
                ...
                <TableCell className="text-muted-foreground">
                 ...
                 <TableCell className="text-muted-foreground text-center">
                  {source.youtubeUrl && source.youtubeUrl !== 'N/A' ? (
                    <a href={source.youtubeUrl} target="_blank" rel="noopener noreferrer" className="hover:text-red-600 inline-block">
