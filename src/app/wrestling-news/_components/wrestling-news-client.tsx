...
import type { WrestlingNewsSource } from "@/data/wrestling-news-sources";
import { Button } from "@/components/ui/button";

interface WrestlingNewsClientProps {
  sources: WrestlingNewsSource[];
}

type SortableColumn = keyof WrestlingNewsSource;
...
export function WrestlingNewsClient({ sources }: WrestlingNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<WrestlingNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
        bValue = b[sortColumn as keyof Omit<WrestlingNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Wrestling News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top wrestling news sources from around the world.
        </p>
      </div>
