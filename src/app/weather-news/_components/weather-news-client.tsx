...
import type { NewsSite as WeatherNewsSource } from "@/data/all-news-sources";
import { Button } from "@/components/ui/button";

interface WeatherNewsClientProps {
  sources: WeatherNewsSource[];
}

type SortableColumn = keyof WeatherNewsSource;
...
export function WeatherNewsClient({ sources }: WeatherNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<WeatherNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
        bValue = b[sortColumn as keyof Omit<WeatherNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Weather News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top weather news sources from around the world.
        </p>
      </div>
