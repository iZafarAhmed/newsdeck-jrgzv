...
import type { NewsSite as TennisNewsSource } from "@/data/all-news-sources";
import { Button } from "@/components/ui/button";

interface TennisNewsClientProps {
  sources: TennisNewsSource[];
}

type SortableColumn = keyof TennisNewsSource;
...
export function TennisNewsClient({ sources }: TennisNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<TennisNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
        bValue = b[sortColumn as keyof Omit<TennisNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Tennis News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top tennis news sources from around the world.
        </p>
      </div>
