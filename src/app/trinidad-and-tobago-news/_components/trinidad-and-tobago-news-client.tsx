...
import type { TrinidadAndTobagoNewsSource } from "@/data/trinidad-and-tobago-news-sources";
import { Button } from "@/components/ui/button";

interface TrinidadAndTobagoNewsClientProps {
  sources: TrinidadAndTobagoNewsSource[];
}

type SortableColumn = keyof TrinidadAndTobagoNewsSource;
...
export function TrinidadAndTobagoNewsClient({ sources }: TrinidadAndTobagoNewsClientProps) {
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<TrinidadAndTobagoNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
        bValue = b[sortColumn as keyof Omit<TrinidadAndTobagoNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers' | 'facebookUrl' | 'xUrl' | 'instagramUrl' | 'country' >] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Trinidad And Tobago News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top news sources from Trinidad and Tobago.
        </p>
      </div>
