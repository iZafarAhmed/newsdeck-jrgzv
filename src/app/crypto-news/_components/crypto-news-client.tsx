...
import type { NewsSite as CryptoNewsSource } from "@/data/all-news-sources";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";

interface CryptoNewsClientProps {
  sources: CryptoNewsSource[];
}

type SortableColumn = keyof CryptoNewsSource;
  ...
  const filteredAndSortedSources = useMemo(() => {
    ...
    const sorted = [...filtered].sort((a, b) => {
      ...
      if (['facebookFollowers', 'xFollowers', 'instagramFollowers'].includes(sortColumn)) {
        ...
        aValue = a[sortColumn as keyof Omit<CryptoNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
        bValue = b[sortColumn as keyof Omit<CryptoNewsSource, 'facebookFollowers' | 'xFollowers' | 'instagramFollowers'>] ?? '';
      }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold tracking-tight">Cryptocurrency News Websites</h1>
        <p className="text-lg text-muted-foreground mt-2">
          Explore top cryptocurrency news sources from around the world.
        </p>
      </div>
          ...
          <TableBody>
            {filteredAndSortedSources.map((source, index) => (
              <TableRow key={source.name} className="hover:bg-muted/50">
                ...
                <TableCell className="text-muted-foreground">
                  {source.instagramUrl && source.instagramFollowers !== 'N/A' ? (
